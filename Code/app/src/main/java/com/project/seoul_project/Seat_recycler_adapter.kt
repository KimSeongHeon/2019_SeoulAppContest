...
class Seat_recycler_adapter (var sdata: ArrayList<Seat_info>)
    : RecyclerView.Adapter<Seat_recycler_adapter.ViewHolder>() { // will be error if no inner class(ViewHolder) defined
    ...
    interface OnItemClickListner {
        fun OnItemClick(holder: ViewHolder, view: View, data: Seat_info, position: Int)
    }
    ...
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): ViewHolder { // inflate layout and return
        ...
        val v = LayoutInflater.from(p0.context).inflate(R.layout.seat_list_card, p0, false)
        context = p0.context
        return ViewHolder(v)
    }
    ...
    override fun onBindViewHolder(p0: ViewHolder, p1: Int) {
        p0.dest.text = sdata.get(p1).dest
        p0.room.text = sdata.get(p1).room
        p0.seat.text = sdata.get(p1).seat
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        init {
            ...
            detail = itemView.findViewById(R.id.recieve_seatdetail_button)
            room = itemView.findViewById(R.id.receive_seatroom_textview)
            dest = itemView.findViewById(R.id.receive_seatdest_textview)
            seat = itemView.findViewById(R.id.receive_seatseat_textview)
            layout = itemView.findViewById(R.id.card_seatlayoutSeat)
            turn = itemView.findViewById(R.id.card_seatturn_imagebutton)
            frame = itemView.findViewById(R.id.card_seatframe)
            itemView.setOnClickListener {
                val position = adapterPosition
                ItemClickListener?.OnItemClick(this, it, sdata[position], position)
            }
        }
            ...
            if(flag == 0){
                ...
                for (i in 0.. seats.length-1) {
                    if (seats.get(i)== '/') {
                        ...
                        view.setTag(2);
                        if(sdata.get(position).seat == count.toString()){
